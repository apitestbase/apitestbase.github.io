...
---
Your test data, settings, etc. live under the `<ATB_DATA_DIR>` folder (refer to [Administration](/docs/en/administration) for the location), and stay untouched during ATB upgrade.
...
For the installer (setup exe), running a new version installer will automatically uninstall the old ATB version and install the new version.
...
Run the new version following the instructions on [Quick Start](/docs/en/quick-start) page, using the same host data folder in the `-v` parameter as before, so that the new container sees your existing data.
...
Extract the downloaded zip, and run the new version following the instructions on [Quick Start](/docs/en/quick-start) page.
