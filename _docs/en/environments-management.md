...
Secrets can be defined only on an environment, not on a workspace. When tests run in a CI/CD pipeline — where no local `secrets.properties` exists — secret values are supplied to the runner at run time instead of being committed; see [Integration Unit Testing Automation in CI/CD Pipeline](/docs/en/integration-unit-testing-automation-in-cicd-pipeline) for how.
...
Properties and managed endpoints can be defined either on an environment or on the workspace itself (open the workspace from the `Workspace Metadata` icon in the left sidebar). Secrets, as noted above, are environment-only.
