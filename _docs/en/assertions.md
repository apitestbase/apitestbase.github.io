...
API Test Base provides a range of assertion types. Most are self-explanatory, so this page covers only the ones with behavior worth explaining.
...
## Ad Hoc Verification
Assertions can also be used as ad hoc utilities, without running a test step or test case. For example, to validate a JSON against a JSON Schema, create an HTTP step in an ad hoc test case and use the JSONValidAgainstJSONSchema assertion directly.
...
To verify an assertion without running the test step, right click in the response editor on ATB UI, select `Toggle Read Only` from the context menu, edit the response, select the assertion in the `Assertions` panel, and click the `Verify` button.